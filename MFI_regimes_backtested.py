...
def process_stock(args):
    symbol, start_date, end_date, mfi_period, max_hold = args
    try:
        ...
        return None
def mfi_search(end_date, mfi_periods, max_hold_periods):
    start_date = (pd.to_datetime(end_date) - pd.DateOffset(years=25)).strftime('%Y-%m-%d')
    symbols = get_sp500_nasdaq100_symbols()
    logger.info(
        f"Parameters: End Date: {end_date}, Start Date: {start_date}, MFI Periods: {mfi_periods}, Max Hold Periods: {max_hold_periods}")
    args = [(symbol, start_date, end_date, mfi_period, max_hold)
...
if __name__ == "__main__":
    ...
    mfi_periods = [20]
    max_hold_periods = [1]
    mfi_search(end_date, mfi_periods, max_hold_periods)
