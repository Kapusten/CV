...
POPULAR_ETFS = ['SPY', '^NDX', '^GSPC', 'QQQ', 'TQQQ', 'MSFT', 'AAPL', 'NVDA', 'AMZN', 'GOOGL', 'META', 'AVGO']
data_cache = {}
...
def mfi_search(end_date, mfi_periods, mfi_trigger, max_hold_periods):
    ...
    logger.info(
        f"Parameters: End Date: {end_date}, Start Date: {start_date}, "
        f"MFI Periods: {mfi_periods}, MFI Trigger: {mfi_trigger}, "
        f"Max Hold Periods: {max_hold_periods}"
    )
    args = [
        (symbol, start_date, end_date, mfi_period, mfi_trigger, max_hold)
    ...
    for (mfi_period, max_hold), period_hold_results in results_by_period_and_hold.items():
        ...
        avg_time_invested = np.mean([r['Time Invested'] for r in period_hold_results])
        ...
        logger.info(f"Average Time Invested: {avg_time_invested:.2f}%")
        summary_row = {
            'MFI_Period': mfi_period,
            'Max_Hold': max_hold,
            ...
            'Average_Time_Invested': avg_time_invested
        }
        summary_rows.append(summary_row)
    with open('mfi_strategy_summary.csv', 'w', newline='') as f:
        if summary_rows:
    ...
    return results_by_period_and_hold
if __name__ == "__main__":
    multiprocessing.freeze_support()
    end_date = '2024-10-31'
    mfi_periods = [1, 2, 3, 4, 5]
    max_hold_periods = [1,2,3,4,5]
