...
def write_to_csv(data, filename, mode='a'):
    with open(filename, mode, newline='') as csvfile:
        fieldnames = ['Symbol', 'Entry Date', 'Entry Price', 'Current Price', 'Current Trade Return', 'Strategy Return', 'Max Drawdown', 'Buy and Hold Return', 'Avg Profit Per Trade', 'Hold Period', 'MFI Period', 'Max Hold']
...
def process_stock(args):
    symbol, start_date, mfi_period, mfi_trigger, max_hold = args
    try:
        ...
        return None
def mfi_search(start_date, mfi_periods, mfi_trigger, max_hold_periods):
    symbols = get_sp500_nasdaq100_symbols()
    logger.info(f"Parameters: MFI Periods: {mfi_periods}, MFI Trigger: {mfi_trigger}, Max Hold Periods: {max_hold_periods}")
    args = [(symbol, start_date, mfi_period, mfi_trigger, max_hold)
    ...
    results_by_period_and_hold = {(period, hold): [] for period in mfi_periods for hold in max_hold_periods}
    for result in results:
        if result is not None:
            period = result['MFI_Period']
            hold = result['Max_Hold']
            results_by_period_and_hold[(period, hold)].append(result)
    all_specific_trades = []
    for period in mfi_periods:
        for max_hold in max_hold_periods:
            period_hold_results = results_by_period_and_hold[(period, max_hold)]
            recent_exits = identify_recent_exits(period_hold_results, current_date)
            open_trades = [trade for trade in period_hold_results if
            ...
            specific_trades = [
                ...
                if 1 <= trade['Hold Period'] < 7 and trade['Strategy Return'] >= 70 and trade['Current Trade Return'] < -0.35 and trade['Max Drawdown'] < 30
            ]
    ...
    if all_specific_trades:
        filename = f"MFI_trendFollowing_{current_date.strftime('%Y%m%d')}_{mfi_trigger}.csv"
...
if __name__ == "__main__":
    ...
    mfi_search(start_date, mfi_periods, 80, max_hold_periods)