...
def write_to_csv(data, filename, mode='a'):
    with open(filename, mode, newline='') as csvfile:
        fieldnames = ['Symbol', 'Entry Date', 'Entry Price', 'Current Price', 'Current Trade Return', 'Strategy Return', 'Max Drawdown', 'Buy and Hold Return', 'Avg Profit Per Trade', 'Hold Period', 'RS Period', 'Holding Period']
...
def calculate_relative_strength(data, index_data, period):

    data['pct_change'] = data['Close'].pct_change()
    index_data['pct_change'] = index_data['Close'].pct_change()


    data['cum_return'] = (1 + data['pct_change']).rolling(window=period).apply(np.prod) - 1
    index_data['cum_return'] = (1 + index_data['pct_change']).rolling(window=period).apply(np.prod) - 1

    relative_strength = data['cum_return'] / index_data['cum_return']

    return relative_strength


def relative_strength_strategy(data, index_data, rs_period, holding_period):
    data['RS'] = calculate_relative_strength(data, index_data, rs_period)
    data['Signal'] = 0
    data['ExitSignal'] = 0
    data['HoldPeriod'] = 0
    in_trade = False
    ...
    for i in range(1, len(data)):
        if data['RS'].iloc[i] > 1 and not in_trade:
        ...
        if data['HoldPeriod'].iloc[i] >= holding_period:
...
def backtest_strategy(data, holding_period):
    position = 0
    entry_price = 0
    equity_curve = [1.0]
    max_equity = 1.0
...
def process_stock(args):
    symbol, start_date, rs_period, holding_period, index_data = args
    try:
        data = fetch_data(symbol, start_date)
        if data is None or len(data) < 100:
            return None

        data = relative_strength_strategy(data, index_data, rs_period, holding_period)
...
def relative_strength_search(start_date, rs_periods, holding_periods):
    symbols = get_sp500_nasdaq100_symbols()

    logger.info(f"Parameters: RS Periods: {rs_periods}, Holding Periods: {holding_periods}")


    index_data = fetch_data('^NDX', start_date)
    if index_data is None:
        logger.error("Failed to fetch ^NDX data. Exiting.")
        return

    args = [(symbol, start_date, rs_period, holding_period, index_data)
            for symbol in symbols
            for rs_period in rs_periods
    ...
    results_by_period = {(rs_period, holding_period): [] for rs_period in rs_periods for holding_period in holding_periods}
    for result in results:
        if result is not None:
            period = result['RS_Period']
            holding = result['Holding_Period']
            results_by_period[(period, holding)].append(result)

    all_specific_trades = []

    for rs_period in rs_periods:
        for holding_period in holding_periods:
            period_results = results_by_period[(rs_period, holding_period)]
            ...
            logger.info(f"\n--- Results for RS Period {rs_period} and Holding Period {holding_period} ---")
            ...
            specific_trades = [
                ...
                if 1 <= trade['Hold Period'] <= (holding_period / 2) and trade['Strategy Return'] >= 70 and trade[
                    'Current Trade Return'] < 1.00 and trade['Max Drawdown'] < 30 and trade['Current Trade Return'] > -4.00
            ]
            ...
            for trade in open_trades:
                ...
                logger.info(f"Unrealized Return: {trade['Unrealized Return']:.2f}%")
    ...
    if all_specific_trades:
        filename = f"RS_strategy_{current_date.strftime('%Y%m%d')}.csv"
...
if __name__ == "__main__":
    ...
    relative_strength_search(start_date, rs_periods, holding_periods)