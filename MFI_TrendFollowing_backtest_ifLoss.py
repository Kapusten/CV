...
file_handler = logging.FileHandler('mfi_trendFollowing_backtested_newEntry.txt')
...
def mfi_strategy(data, mfi_period, mfi_trigger, max_hold):
    ...
    in_trade = False
    potential_entry_price = 0
    for i in range(1, len(data)):
        if data['MFI'].iloc[i] > mfi_trigger and not in_trade and potential_entry_price == 0:
            data.loc[data.index[i], 'PotentialEntry'] = 1
            potential_entry_price = data['Close'].iloc[i]
        elif potential_entry_price > 0 and not in_trade:
            current_drawdown = (potential_entry_price - data['Close'].iloc[i]) / potential_entry_price
            if current_drawdown >= 0.0035:
...
def backtest_strategy(data, max_hold):
    ...
    for i in range(1, total_days):
        if data['PotentialEntry'].iloc[i] == 1 and position == 0:
            potential_entry_price = data['Close'].iloc[i]
