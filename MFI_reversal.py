...
def write_to_csv(data, filename, mode='a'):
    with open(filename, mode, newline='') as csvfile:
        fieldnames = ['Symbol', 'Entry Date', 'Entry Price', 'Current Price', 'Current Trade Return', 'Strategy Return', 'Max Drawdown', 'Buy and Hold Return', 'Avg Profit Per Trade', 'Hold Period', 'MFI Period']
...
file_handler = logging.FileHandler('mfi_reversal.txt')
...
def process_stock(args):
    ...
    try:
        ...
        return None
def mfi_search(start_date, mfi_periods, mfi_trigger, max_hold):
    symbols = get_sp500_nasdaq100_symbols()
    logger.info(f"Parameters: MFI Periods: {mfi_periods}, MFI Trigger: {mfi_trigger}, Max Hold: {max_hold}")
    ...
    results_by_period = {period: [] for period in mfi_periods}
    for result in results:
    ...
    for period in mfi_periods:
        ...
        open_trades = [trade for trade in period_results if
                       'Strategy Return' in trade and trade['Strategy Return'] > 60 and trade['Max Drawdown'] < 30]
        open_trades.sort(key=lambda x: (-x['Strategy Return']))
        logger.info(f"\n--- Results for MFI Period {period} ---")
        specific_trades = [
            {
                'Symbol': trade['Symbol'],
            ...
            if 1 <= trade['Hold Period'] < 7 and trade['Strategy Return'] >= 60 and trade['Current Trade Return'] < -0.35 and trade['Max Drawdown'] < 30
        ]
    ...
    if all_specific_trades:
        filename = f"MFI_reversal_{current_date.strftime('%Y%m%d')}_{mfi_trigger}_{max_hold}.csv"
...
if __name__ == "__main__":
    ...
    mfi_periods = [2, 3, 4, 5]
    mfi_search(start_date, mfi_periods, 10, 10)